...
*/
public class IBkMetric extends knnClassifier implements
...
/**
 ...
 /* data set que guarda las inatancias ordenadas*/
 
 public Instances m_Original;
  ...
  */
 public IBkMetric(int k, MetricSpace ms) {
 ...
 public void setMs(MetricSpace m) {
 ...
 public void buildClassifier(Instances instances) throws Exception {
   ...
   if (m_DontNormalize) {
	...
	m_Min[i] = m_Max[i] = Double.NaN;
     ...
     Enumeration enum1 = m_Train.enumerateInstances();
     while (enum1.hasMoreElements()) {
	updateMinMax((Instance) enum1.nextElement());
     }
   }

   // Compute the number of attributes that contribute
   // to each prediction 
  // usar tambien numerales numericos string
   m_NumAttributesUsed = 0.0;
   for (int i = 0; i < m_Train.numAttributes(); i++) {
     if ((i != m_Train.classIndex()) && 
	  (m_Train.attribute(i).isNominal() ||
	   m_Train.attribute(i).isNumeric() ||
	   m_Train.attribute(i).isString())) {
 ...
 /*protected double distance(Instance first, Instance second, Instances m)
 {
	return m_ClassType;
...
/* weka orignal code  for distance function distance  */
...
protected double distance(Instance first, Instance second) {  
   ...
   int firstI, secondI;
//aqui devuelve el numero de atributos con numvalues
   
   // reviso  si la inatncia es de tipo String
   for (int p1 = 0, p2 = 0;  p1 < first.numValues() || p2 < second.numValues();) {
	...
	firstI = m_Train.numAttributes();
     ...
     if (firstI == secondI) {
    	...
    	//System.out.println("f:"+first.valueSparse(p1)+", s:"+second.valueSparse(p2));
...
// busqueda recursiva //
 ...
 protected double differenceFromString(int  index,double first, double second)
 {
	 
	 // se carga la matriz de distancia  con los valdo calculados de la distancia de edicion
	 Instances DS=m_Train;
		...
		String dsString;
			...
			int i=0;
			//Veriifico q las instancias sean diferentes
			if (first!=second)
			{
			...
			//Se toma de la matrix de distancia el valor de x,y la matriz de distancia para el atributo
			double[][] p=(double[][]) (ms.getMetricSpace()).elementAt(index);
		...
		/* normalizacion usando min-max*/
		diff=((diff - min) / (max -min));
		
		
		//diff=(diff/Math.pow(10, 2));
		
		//weka.filters.unsupervised.attribute.= weka.filters.unsupervised.attribute.ma
		
		/*para equilibrar el valor de la particion*/
	if (diff < 0.5) {
		    diff = 1.0 - diff;
		  }
		  
		// weka.filters.unsupervised.attribute.Standardize flt= new weka.filters.unsupervised.attribute.Standardize();
		 //flt.
		}
				
		System.out.println("valor de  diff"+diff);
 ...
 public double vecMenor(double [] arreglo)
 {
 ...
 */
 public double vecMayor( double[] arreglo)
 {
 ...
 */
 ...
 protected double difference(int index, double val1, double val2) {
	 ...
	 /* imprime el dataser por pantalla*/
	// View.viewData(m_Train);
	 if((m_Train.attribute(index).type())==Attribute.STRING)	 
	 {
		 System.out.println("atributo tipo cadena");
	 }
   ...
   case Attribute.NOMINAL:
     ...
     if (Instance.isMissingValue(val1) || 
	  ...
	  ((int)val1 != (int)val2)) {
    	// System.out.println(" valores numericos " +(int)val1+" "+(int)val2);
	return 1;
     } else {
	return 0;
     }
 ...
 case Attribute.STRING:
    	 ...
    	 double VN=0;
    	VN=differenceFromString(index,val1,val2);
	//norm(VN,index);
	//m_Min[index];
    	return VN;//
 ...
 protected double norm(double x, int i) {
   ...
   if (m_DontNormalize) {
	 ...
	 //  System.out.println("min"+m_Min[i]+" max"+m_Max[i]);
 ...
 protected NeighborList findNeighbors(Instance instance) {
   ...
   while (enum1.hasMoreElements()) {
     Instance trainInstance = (Instance) enum1.nextElement();
   ...
   if (!neighborlist.isEmpty()) {
	...
	try {
	  ...
	  case Attribute.NUMERIC:
	    ...
	    /*jl se hace la modificacion para dar ponderacion a los atributos*/
	  case Attribute.STRING:
		    distribution[0] += current.m_Instance.classValue() * weight;
		    break;
	  }
 ...
 public static void main(String [] argv) {
   ...
   try {
     System.out.println(Evaluation.evaluateModel(new IBkMetric().toString(), argv));
   } catch (Exception e) {
     e.printStackTrace();
